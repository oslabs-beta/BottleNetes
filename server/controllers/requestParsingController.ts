...
import { RequestParsingController } from "controller-types";
...
const requestParsingController: RequestParsingController = {
  parseRequestAllPodsStatus: (_req, _res, next) => {
    return next();
  },

  parseRequestAllPodsRequestLimit: (_req, _res, next) => {
    return next();
  },

  parseRequestResourceUsageOneValue: (req, res, next) => {
  ...
  parseRequestResourceUsageHistorical: (req, res, next) => {
    if (
      !req.body.type ||
      !req.body.timeStart ||
      !req.body.timeEnd ||
  ...
  parseRequestLatencyAppRequestOneValue: (req, res, next) => {
    if (!req.body.time.match(/^(\d+)(s|m|h|d)$/)) {
      return next({
  ...
  parseRequestLatencyAppRequestHistorical: (req, res, next) => {
    if (
      !req.body.timeStart ||
      !req.body.timeEnd ||
      !req.body.timeStep ||
...
export default requestParsingController;
